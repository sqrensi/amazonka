...
{
    float sharp = max(_TerrainTriplanarSharp, 1.0);
    float3 b = pow(abs(n) + 1e-5, sharp);
    return b / dot(b, 1.0);
}

float2 TerrainTriUV(float3 wp, float4 st, int axis)
{
    float3 size = max(_TerrainWorldSize, float3(1, 1, 1));
    float scale = 0.5 * (st.x / size.x + st.y / size.z);
    if (axis == 0)
        return float2(wp.z, wp.y) * scale + st.zw;
    if (axis == 2)
        return float2(wp.x, wp.y) * scale + st.zw;
    return float2(wp.x, wp.z) * scale + st.zw;
}

half4 TerrainTriplanarSample(Texture2D tex, SamplerState samp, float3 wp, float3 wn, float4 st, float2 planarUv)
{
    if (_TerrainWorldSize.x < 0.5)
        return SAMPLE_TEXTURE2D(tex, samp, planarUv);
    float3 b = TerrainTriBlend(wn);
    return SAMPLE_TEXTURE2D(tex, samp, TerrainTriUV(wp, st, 0)) * b.x
         + SAMPLE_TEXTURE2D(tex, samp, TerrainTriUV(wp, st, 1)) * b.y
         + SAMPLE_TEXTURE2D(tex, samp, TerrainTriUV(wp, st, 2)) * b.z;
}
...
void NormalMapMix(float4 uvSplat01, float4 uvSplat23, float3 positionWS, float3 normalWS, inout half4 splatControl, inout half3 mixedNormal)
{
    #if defined(_NORMALMAP)
        half3 nrm = half(0.0);
        nrm += splatControl.r * UnpackNormalScale(TerrainTriplanarSample(_Normal0, sampler_Normal0, positionWS, normalWS, _Splat0_ST, uvSplat01.xy), _NormalScale0);
        nrm += splatControl.g * UnpackNormalScale(TerrainTriplanarSample(_Normal1, sampler_Normal0, positionWS, normalWS, _Splat1_ST, uvSplat01.zw), _NormalScale1);
        nrm += splatControl.b * UnpackNormalScale(TerrainTriplanarSample(_Normal2, sampler_Normal0, positionWS, normalWS, _Splat2_ST, uvSplat23.xy), _NormalScale2);
        nrm += splatControl.a * UnpackNormalScale(TerrainTriplanarSample(_Normal3, sampler_Normal0, positionWS, normalWS, _Splat3_ST, uvSplat23.zw), _NormalScale3);
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, float3 positionWS, float3 normalWS, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    half4 diffAlbedo[4];

    diffAlbedo[0] = TerrainTriplanarSample(_Splat0, sampler_Splat0, positionWS, normalWS, _Splat0_ST, uvSplat01.xy);
    diffAlbedo[1] = TerrainTriplanarSample(_Splat1, sampler_Splat0, positionWS, normalWS, _Splat1_ST, uvSplat01.zw);
    diffAlbedo[2] = TerrainTriplanarSample(_Splat2, sampler_Splat0, positionWS, normalWS, _Splat2_ST, uvSplat23.xy);
    diffAlbedo[3] = TerrainTriplanarSample(_Splat3, sampler_Splat0, positionWS, normalWS, _Splat3_ST, uvSplat23.zw);
    ...
    NormalMapMix(uvSplat01, uvSplat23, positionWS, normalWS, splatControl, mixedNormal);
}
...
#ifdef _MASKMAP
    masks[0] = lerp(masks[0], TerrainTriplanarSample(_Mask0, sampler_Mask0, IN.positionWS, IN.normal.xyz, _Splat0_ST, IN.uvSplat01.xy), hasMask.x);
    masks[1] = lerp(masks[1], TerrainTriplanarSample(_Mask1, sampler_Mask0, IN.positionWS, IN.normal.xyz, _Splat1_ST, IN.uvSplat01.zw), hasMask.y);
    masks[2] = lerp(masks[2], TerrainTriplanarSample(_Mask2, sampler_Mask0, IN.positionWS, IN.normal.xyz, _Splat2_ST, IN.uvSplat23.xy), hasMask.z);
    masks[3] = lerp(masks[3], TerrainTriplanarSample(_Mask3, sampler_Mask0, IN.positionWS, IN.normal.xyz, _Splat3_ST, IN.uvSplat23.zw), hasMask.w);
#endif
